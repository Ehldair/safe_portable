        ...
        if(mysqli_num_rows($resultado_sujeto)==0) {
            if(isset($_POST['detenido']) or ($entera==1) or ($todos_datos==1)) {
                ...
                while ($line_evidencias = mysqli_fetch_array($resultado_evidencias, MYSQLI_ASSOC)) {
                    ...
                    $sql_clonado= "select * from evidencia_registro where (id_evidencia=$id_evidencia and id_estado_evidencia=2) or (id_evidencia=$id_evidencia and id_estado_evidencia=1) order by fecha_alta_estado desc";
                    $resultado_clonado=mysqli_query($link, $sql_clonado);
                    $count_clonado=mysqli_num_rows($resultado_clonado);
                    $contador=0;

                    while ($line_clonado = mysqli_fetch_array($resultado_clonado, MYSQLI_ASSOC)) {
                        $id_accion_programa=$line_clonado['id_accion_programa'];
                        $id_programa=$line_clonado['id_programa'];
                        $id_estado_evidencia=$line_clonado['id_estado_evidencia'];
                        
                        if($contador==0) {
                           
                            if(isset($_POST['clonado']) or ($entera==1) or ($todos_datos_evidencias==1)) {
                                
                                if($count_clonado!=0 and $id_estado_evidencia!=1) {
                            ...
                            $pdf->SetXY(197,$y+7);
                            $pdf->Cell(2,2,'',1,0,'C');
                            if($id_accion_programa==6 or $id_accion_programa==7) {
                                $pdf->Cell(-2,2,'X',0,0,'C');
                            }
                            $pdf->SetXY(181,$y+7);
