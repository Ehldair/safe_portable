...
$myid_comisaria=base64_decode(mysqli_real_escape_string($link,$_GET['id_comisaria']));
$_SESSION['id_comisaria']=$myid_comisaria;

$sql = "SELECT  ca.nombre_ca, c.id_ca, p.nombre_provincia, c.id_provincia,c.nombre_comisaria,id_comisaria FROM  comisaria c
INNER JOIN PROVINCIA p on p.id_provincia=c.id_provincia
INNER JOIN CA ca on ca.id_ca=c.id_ca WHERE id_comisaria=$myid_comisaria";
...
<html lang="es-ES">
<head>
 <title>Modificar Comisaria</title>
	<meta charset="utf-8" />
...
<body class="is-preload">
	...
	<!-- Main -->
    				<section id="main" class="container">
    					...
    					<header>
    						<h2>Modificar Comisaria</h2>						
    					</header>
    					
    					
    					<div class="box">
    						<form method="post" id="myform" action="crearcomisaria.php?mod=1">
    							Modificar una comisaria <br><br>
    							<div align="center">
        ...
        <?php
            ...
            echo "<option value=$ret[id_ca]>$ret[nombre_ca]</option>";
		...
		<?php
            ...
            echo "<option value=$ret[id_provincia]>$ret[nombre_provincia]</option>";
            $resultado = mysqli_query($link, "select id_provincia,nombre_provincia FROM provincia where id_ca=$ret[id_ca] and id_provincia!=$ret[id_provincia]");
            $contador = 0;
            while ($line = mysqli_fetch_array($resultado, MYSQLI_ASSOC)) {
                foreach ($line as $col_value) {
                    if ($contador == 0) {
        ...
        ?>		
   		...
   		<?php 
   		echo "Comisaria:";
   		echo "<input type='text' name='comisaria' id='comisaria' value='$ret[nombre_comisaria]' placeholder='Comisaría'>";
   		echo "<input type='hidden' name='id_comisaria' id='id_comisaria' value='$ret[id_comisaria]'>";
   		?>
   		</div>
		<div class="col-12">
    							<ul class="actions special">
    								<li><input type="submit" id="alta" value="Modificar Comisaria"/></li>	
    								<li><input type="button" onclick="location.href='gestion_comisaria.php';" value="Volver"><br></li>							
    							</ul>
    						</div>
    						</div>
    						</div>
