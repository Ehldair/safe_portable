...
if(isset($_SESSION['id_u'])) {
    ...
    <head>
     <title>Añadir días</title>
    	...
    	<script>
    	function cambiarUsuarios(opSelect){
    		
    		var category=opSelect;
	    	var url="obtenerusuarios_viaje.php";
	    	var pro= $.ajax({
		...
		function cambiar(opSelect){
    		...
    		var category=opSelect;
    		if(category!=1) {
        		document.getElementById("viaje").disabled=true;
        		...
        		document.getElementById("viaje").disabled=false;
    					...
    					<div class="box">
    						<form method="post" id="myform" action="javascript:validar();">
    							...
    							<div class="row gtr-50 gtr-uniform">
									<div class="col-12 col-12-mobilep">
										<input type="radio" name="tipo" id="tipo1" value="1" onchange="cambiar(this.value);" required> <label for="tipo1">Por Viaje</label>
										<input type="radio" name="tipo" id="tipo2" value="2" onchange="cambiar(this.value);" required> <label for="tipo2">Por Incidencias</label>
										<input type="radio" name="tipo" id="tipo3" value="3" onchange="cambiar(this.value);" required> <label for="tipo3">Otras</label>
									</div>
    								...
    								<div class="col-4 col-12-mobilep">
    								<?php 
    									echo "<select id='viaje' name='viaje' onchange='cambiarUsuarios(this.value);'required> ";
    									$sql="select id_viajes,ciudad from viajes v
                                        INNER JOIN caso c on c.id_caso=v.id_caso
                                        ORDER BY c.año DESC,fecha_alta_caso DESC
                                        LIMIT 10";
    									$result=mysqli_query($link, $sql);
    									echo "<option value=''>Elige el viaje</option>";
    									while ($line = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
    									    $id_viajes=$line['id_viajes'];
    									    $ciudad=$line['ciudad'];
    									    
    									    echo "<option value='$id_viajes'>$ciudad</option>";
    									}
    									echo "</select>";
    								?>
    								</div>
    						...
    						<div class="col-12">
    							<ul class="actions special">
    								<li><input type="submit" id="alta" value="Alta Días"/></li>	
    								<li><input type="button" onclick="location.href='gestion_dias.php';" value="Volver"><br></li>							
    							</ul>
    						</div>
