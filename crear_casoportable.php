...
if(isset($_SESSION['id_u'])) {
    ...
    $myid_hash='Xid_hashX';
    $comprobador_intervenciones_sinsujeto=0;
    ...
    if($count_sujeto!=0) {
        ...
        //comprobamos si existen intervenciones sin sujeto y no se han añadido ya, en un  caso que tiene mas intervenciones que sujetos y en su caso se añaden aqui
        if($comprobador_intervenciones_sinsujeto==0) {
            $sql="select * from intervencion where id_caso=$myid_caso and (id_sujeto_activo=1)";
            $result_intervencion=mysqli_query($link_portable, $sql);
            $count_intervencion=mysqli_num_rows($result_intervencion);
            if($count_intervencion!=0) {
